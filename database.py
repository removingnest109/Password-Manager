...
def login():
    ...
    def check_password():
        ...
        if config.has_option('keys', 'user_password') and config.get('keys', 'user_password') == password:
            root.destroy()
            main()
        else:
...
if not config.has_option('keys', 'user_password'):
    ...
    def set_password():
        password = password_entry.get()
        config.set('keys', 'user_password', password)
        with open(config_file, 'w') as f:
            config.write(f)
        root.destroy()
